...
def ordered_search(index, ranks, keyword):
    if keyword in index:
        ...
        for url in url_list:
            outcome[url] = ranks[url]
        outcome_list = list(outcome.items())
        # print(outcome_list)
        return quick_sort(outcome_list)


def quick_sort(outcome):
    if len(outcome) == 1:
        for i, j in outcome:
            return [i]
    elif outcome == []:
        ...
        pivot_link, pivot_rank = outcome[0]
        left, right = [], []
        for link, rank in outcome[1:]:
            if rank < pivot_rank: #sorting ranks from the highest down
                right.append((link, rank))
            else:
                left.append((link, rank))

        if left != [] and right != []:
            return quick_sort(left) + [pivot_link] + quick_sort(right)
        elif left == []:
            return [pivot_link] + quick_sort(right)
        else:
            return quick_sort(left) + [pivot_link]

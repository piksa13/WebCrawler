...
def cellular_automaton(string, pattern_no, n):
    pattern_numbers = [1, 2, 4, 8, 16, 32, 64, 128]
    pattern_list = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']
    pattern_dict = {}

    for i in range(len(pattern_numbers) - 1, -1, -1):
        if pattern_numbers[i] <= pattern_no:
            pattern_dict[pattern_list[i]] = 'x'
            pattern_no = pattern_no - pattern_numbers[i]
        else:
            pattern_dict[pattern_list[i]] = '.'

    for j in range(n):
        new_string = ''
        for k in range(len(string)):
            pattern = string[k - 1] + string[k] + string[(k + 1) % len(string)]
            new_string = new_string + pattern_dict[pattern]
        string = new_string
    return new_string
